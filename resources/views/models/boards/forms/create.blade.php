                    ...
                    <div class="card-body">
                        ...
                        <div class="d-flex justify-conten-between">
                            <div class="col-6 pl-0 form-group{{ $errors->has('first_name') ? ' has-error' : '' }}">
                                ...
                                <select class="custom-select" name="layout" id="layout">
                                    @foreach( config('platform.database.boards.layouts.options') as $key => $val )
                                    <option value="{{ $key }}" @if( $key == config('platform.database.layouts.default')) @endif >{{ $val }}</option>
                                    @endforeach     
                                </select>
                             ...
                             <div class="col-6 pr-0">
                                ...
                                <select class="custom-select" name="status" id="status">
                                    @foreach( config('platform.database.boards.status.options') as $key => $val )
                                    <option value="{{ $key }}" @if( $key == config('platform.database.status.default')) @endif >{{ $val }}</option>
                                    @endforeach     
                                </select>
                            </div>
                        </div>
