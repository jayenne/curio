...
<article id="grid-post-{{ $item['id'] }}" class="grid-item col-sm-12 col-md-4 col-xl-3" data-id="{{ $item['id']}}" data-total-weight="{{-- $item['reactions']['totals']['weight'] --}}" data-total-count="{{-- $item['reactions']['totals']['count'] --}}" data-type="{{ $item['type'] }}" data-theme="{{ $item['theme'] }}">
    <div class="card" data-toggle="modal" data-toggle="modal" data-target="#modal-griditem" data-url="p/{{ $item['id'] }}">
        <div class="card-img-top">
            @if($item['media']['data'] > 0)
                @foreach($item['media']['data'] as $key => $element)
                    <img src="{{ $element['medium'] }}" class="img-{{ $loop->iteration }} of-{{ $loop->count }}" alt="{!! $item['title'] !!}" onerror='this.onerror = null; this.src="{{ asset(config('platform.media.posts.small.missing')) }}"' />
        ...
        <div class="card-body d-flex flex-row">
            ...
            <div class="card-stats d-flex flex-column justify-content-start align-items-center">   
                ...
                <img class="avatar mb-2 justify-content-start align-self-center" src="{{ $item['user']['profile']['avatar'] }}" title="{{ $item['user']['name'] }}" data-toggle="tooltip" onerror='this.onerror = null; this.src="{{ asset(config('platform.fallback.user.grid.avatar')) }}"' />
